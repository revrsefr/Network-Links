...
<!--
[![Latest stable release](https://img.shields.io/github/v/tag/jlu5/pylink?label=stable&color=1a1)](https://github.com/Network-Links/Network-Links/tree/master)
...
Network-Links is an extensible, plugin-based IRC services framework written in Python. It aims to be:
...
Please report any bugs you find to the [issue tracker](https://github.com/reversefr/Network-Linkss/issues). Pull requests are likewise welcome.

## Installation

### Pre-requisites
* Python 3.7 or above - prefer the newest Python 3.x when available
* A Unix-like operating system: Network-Links is actively developed on Linux only, so we cannot guarantee that things will work properly on other systems.

If you are a developer and want to help make Network-Links more portable, patches are welcome.
...
2) Clone the repository: `git clone https://github.com/Network-Links/Network-Links && cd Network-Links`
    - Previously there was a *devel* branch for testing versions of Network-Links - this practice has since been discontinued.

3) Install Network-Links using `python3 setup.py install` (global install) or `python3 setup.py install --user` (local install)
    * Note: `--user` is a *literal* string; *do not* replace it with your username.
    *  **Whenever you switch branches or update Network-Links's sources via `git pull`, you will need to re-run this command for changes to apply!**

### Installing via Docker

As of Network-Links 3.0 there is a Docker image available on Docker Hub: [jlu5/pylink](https://hub.docker.com/r/jlu5/pylink)
...
2) Run `pip3 install pylinkirc` to download and install Network-Links. pip will automatically resolve dependencies.

3) Download or copy https://github.com/Network-Links/Network-Links/blob/master/example-conf.yml for an example configuration.

## Configuration

1) Rename `example-conf.yml` to `pylink.yml` (or a similarly named `.yml` file) and configure your instance there.

2) Run `pylink` from the command line. Network-Links will load its configuration from `pylink.yml` by default, but you can override this by running `pylink` with a config argument (e.g. `pylink mynet.yml`).
...
* [Nefarious IRCu](https://github.com/evilnet/nefarious2) (2.0.0+) - module `p10`
    - Note: Both account cloaks (user and oper) and hashed IP cloaks are optionally supported (`HOST_HIDING_STYLE` settings 0 to 3). Make sure you configure Network-Links to match your IRCd settings.
...
* [charybdis](https://github.com/charybdis-ircd/charybdis) (3.5+) - module `ts6`
    - For KLINE support to work, a `shared{}` block should be added for Network-Links on all servers.
...
* [ngIRCd](https://ngircd.barton.de/) (24+) - module `ngircd`
    ...
    - `+` (modeless) channels are not supported, and should be disabled for Network-Links to function correctly.
...
* [beware-ircd](http://ircd.bircd.org/) (1.6.3) - module `p10`
    - Because bircd disallows BURST after ENDBURST for regular servers, U-lines are required for all Network-Links servers. Fortunately, wildcards are supported in U-lines, so you can add something along the lines of `U:<your pylink server>:` and `U:*.relay:` (adjust accordingly for your relay server suffix).
...
* [IRCd-Hybrid](http://www.ircd-hybrid.org/) (8.2.x / svn trunk) - module `hybrid`
    - For host changing support and optimal functionality, a `service{}` block / U-line should be added for Network-Links on every IRCd across your network.
    - For KLINE support to work, a `shared{}` block should also be added for Network-Links on all servers.
...
Network-Links supports connecting to IRCds as a relay bot and forwarding users back as virtual clients, similar to Janus' Clientbot. This can be useful if the IRCd a network used isn't supported, or if you want to relay certain channels without fully linking with a network.
